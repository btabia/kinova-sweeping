...
int main(int argc, char *argv[]) {
  ...
  auto robot = std::make_shared<KinovaRobot>("robot", "192.168.1.111");
  auto robotControl = std::make_shared<KinovaControl>("robot", robot, node);
  auto robotPlanner = std::make_shared<GeneralPlanner>("robot", node);
  ...
  executor.add_node(robotControl->getImpedanceController());

 

  // Set up Loop cycle
  std::shared_ptr<LoopCycle> loopCycle =
      std::make_shared<LoopCycle>(node, robot, robotControl, robotPlanner);
