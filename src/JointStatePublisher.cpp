...
JointStatePublisher::JointStatePublisher(std::shared_ptr<KinovaRobot> k_)
  ...
  std::stringstream gripper_topic_name;
  gripper_topic_name << "/" << k->name << "/gripper_state";
  ...
  gripper_publisher_ = this->create_publisher<std_msgs::msg::Float32>(
      gripper_topic_name.str(), 10);
